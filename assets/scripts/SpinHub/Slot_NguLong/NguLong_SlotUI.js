...
cc.Class({
    ...
    // InitializePool() {
    //     for (let i = 0; i < 5; i++) {
    //         let newNode = cc.instantiate(this.nodeFireItem);
    //         newNode.active = false;
    //         newNode.parent = this.node;
    //         this.fireItemPool.push(newNode);
    //     }
    // },

    getObjectFromPool() {
        if (this.poolFactory) {
            let fire = this.poolFactory.getObject('Fire');
            return fire;
        }
    },

    returnObjectToPool(objectNode) {
        // Trả đối tượng vào pool
        if (objectNode && this.poolFactory) 
            this.poolFactory.removeObject(objectNode);
    },
    ...
    PlaySpinColumn(timeDistanceColumn) {
        ...
        for(let i = 0; i < this.listSpinObj.length; i++)
        {
            let delayedFunction = (index) => {
                return () => {
                    this.listStopObj[index].active = false;
                    this.listSpinObj[index].PlaySpin(this.listStopObj[index]);
                    // this.CheckSpinColumn(this.listSpinObj[index].node.children[0], index); - bonus giữ item thì dùng dòng này
                };
            };
            this.scheduleOnce(delayedFunction(i), i * timeDistanceColumn);
            // this.scheduleOnce(()=>{
    ...
    */
    ...
    SetImageItem(id, itemView, loop = false, dragonId = 1) {
        ...
        //     itemView.getComponent(this.nameClassItem).SetImageItemFake(idItem, this.itemImgDataFree[idItem-1]);  
        // else
            itemView.getComponent(this.nameClassItem).SetImageItemFake(idItem, this.itemImgData[idItem-1]);  
    },
    ...
    SetImageItemReal(id, itemView, loop = false, dragonId = 1) {
        ...
        if(this.CheckIconSpineById(idItem)){
            ...
            if(id == 1)
            {
                let fire = this.getObjectFromPool();
                itemView.getComponent(this.nameClassItem).SetSpineItem(this, idItem, this.itemSpineData[idItem-1], this.itemSpineBGData[idItem-1], 
                    anim, animBG, isGem, fire, () => {
                        this.returnObjectToPool(fire);
                    });  
            }else
                itemView.getComponent(this.nameClassItem).SetSpineItem(this, idItem, this.itemSpineData[idItem-1], this.itemSpineBGData[idItem-1], anim, animBG, isGem);  
        }
        else{
            itemView.getComponent(this.nameClassItem).SetImageItem(this, idItem, this.itemImgData[idItem-1], isGem);  
        }
    },
    ...
    */
    ...
    GetNameSpinById(id, isFree = false){
        ...
        switch(id){
            case 1:
                ...
                break;
            case 2:
                animationName = isFree ?  "animation" : "animation";
                break;
            case 3:
    ...
    ShowMulti(multi, toDoList){
        if(this.multiplyNode)
            this.multiplyNode.emit("SHOW_MULTIPLY", multi, !1, "normalGame", 4, function() {
                toDoList.DoWork();
            })
    },
