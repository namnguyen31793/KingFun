...
cc.Class({
    ...
    ShowPage(page){
        ...
        for(let i = 0; i < lengthRow; i++){
            ...
            if(index < this.dataHistory.length){
                this.listItemHistorySlot[i].initItem2(this.dataHistory[index], this);
            }else{
    ...
    ShowDetailTurn(info){
        if(this.CheckMatrixFree(info.Description))
            return;
    ...
    CheckMatrixFree(Description){
        var listData = Description.split(" - ");
        var matrix = listData[1].split(":")[1];
        let matrixInfo = matrixData.split("|");
        let matrixStr = matrixString[0].split(",");
        if(matrixStr.length != 15)
            return true;
        else
            return false;
    },
