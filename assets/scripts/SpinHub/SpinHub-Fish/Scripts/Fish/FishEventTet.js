...
cc.Class({
    ...
    PlayEffectDie() {
        cc.resources.load('SpinHub-Fish/Prefabs/Fish2/EffectTet', cc.Prefab, function (err, prefab) {      
        //Global.DownloadManager.LoadPrefab("Fish2","EffectTet", (prefab)=>{
            let node = cc.instantiate(prefab);
            Global.InGameManager.FishPoolInstance.OtherContainer.addChild(node);
            node.y = 95;
            node.getComponentInChildren(sp.Skeleton).setAnimation(0, 'animation', false);
            this.scheduleOnce(()=>{
                node.destroy();
            }, 2);
        });
