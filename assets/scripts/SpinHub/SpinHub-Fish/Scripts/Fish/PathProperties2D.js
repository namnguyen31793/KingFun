       ...
       // let pos = this.pointList [countTarget - 1].clone().addSelf(direction.mulSelf(currentTime / (this.timeMoveList [countTarget] - this.timeMoveList [countTarget - 1])));
        ...
        if(isRotate)
        {
            ...
            if (angleValue <= 0 && angleValue >= -180) {
                this.OwnerObject.node.scaleX = 1;
        ...
        {
            let fishGetComponent = this.OwnerObject.getComponent("Fish");
            if(fishGetComponent != null)
            {
                fishGetComponent.Handle_RotateFowardPath(angleValue);
            }
        }
...
module.exports = PathProperties2D;
