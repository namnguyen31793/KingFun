cc.Class({
    ...
    Init2(type , id , isMe, _targetId, heso,fishValue){
        this.getComponent(cc.Collider).enabled = true;
        this.vx = 900 * Math.sin(-this.node.angle * Math.PI / 180);
        this.vy = 900 * Math.cos(-this.node.angle * Math.PI / 180);
        this.type = type;
    ...
    update (dt) {
        ...
        {
            // cc.log("ngang:"+this.lifeTime+"   "+this.limitLeft+"  "+this.limitRight+"    "+x+"   "+vx+"   "+this.vx+"   "+this.node.x);
            this.vx *= -1;
            vx = this.vx * dt;
            x = this.node.x += vx;
            this.lifeTime--;
        ...
        if(fishCollision)
        {
            ...
            if(fishCollision.FishProperties.FishType < 10 &&  this.totalWin < this.fishValue )
            {
                ...
                this.totalWin += (this.fishValue/20);     
                 ;
                Global.InGameManager.SetValueWinElectric(this.killActor, this.totalWin, false);
                }
            }
        }
        let timeWait = 0;
    ...
    EndBullet(fishAffect = null) {
        ...
        if(this.type != Global.Enum.FISH_TYPE_CONFIG.LAZE_GUN_ID) {
            ...
            this.scheduleOnce(()=>{
                ...
                Global.InGameManager.SetValueWinElectric(this.killActor, this.fishValue, true);
    ...
    Handle_ShootingLazes(fishAffect = null,fishValue = 0)
    {
        let timeWait = 0.3;
        ...
        try 
        {
            this.scheduleOnce(()=>{
                ...
                let affectDeathFishList =  require("FishCollection").getIns().GetRandomFishInSceneView (cc.winSize.width/2, 350,fishAffect.length/2);
                //let data = Global.ServerBot.GetListFishDieByMultiElectric(Global.ServerBot.GetSurplusSpecial(this.accountId), fishAffect);
                let listAffectDie = affectDeathFishList;
                let surplus = 0;
                if(this.killActor == null) {
                    this.removeBullet();
                    Global.ServerBot.RemoveSpecialFish(this.accountId);
                ...
                let multiGun = this.killActor.GetMoneyPerShotByCurrentGunId();
                console.log("change gun:"+this.killActor.actorPropertis.CacheGun);
                if(this.killActor != null && this.killActor.actorPropertis != null) {
                    this.killActor.ChangeGun(this.killActor.actorPropertis.CacheGun);
                for(let i = 0; i < affectDeathFishList.length; i++) {
                            ...
                            let fishDie = require("FishCollection").getIns().GetFishById(affectDeathFishList[i].FishProperties.FishId);
                            if(fishDie != null && fishDie.FishProperties != null) {                             
                                let reward = listAffectDie[i].multi* multiGun;                             
                                fishDie.reoveFishHaveEffect (null,this.killActor);                       
                                this.totalWin += reward;
                                Global.ServerBot.ServerKillFish(listAffectDie[i].fishId);
                            }                 
                } 
               
                Global.InGameManager.SetValueWinElectric(this.killActor, fishValue, false);
                this.killActor.UpdateBalance(fishValue, true);
                
                this.scheduleOnce(()=>{
                    Global.ServerBot.RemoveSpecialFish(this.accountId);
                    Global.InGameManager.SetValueWinElectric(this.killActor, fishValue, true);
                    this.removeBullet();
                ...
                this.Handle_OnCanAttack();
            } , timeWait);   
        }  
        catch
        {
            if(this.killActor != null) {
                Global.InGameManager.SetValueWinElectric(this.killActor, this.totalWin, true);
                this.removeBullet();
                Global.ServerBot.RemoveSpecialFish(this.accountId);
                this.killActor.ChangeGun(1);
            } else {
                this.removeBullet();
                Global.ServerBot.RemoveSpecialFish(this.accountId);
            }
